...
<div class="px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
    <h1 class="mb-6 text-3xl font-bold text-gray-900">{{ __('cart.title') }}</h1>

    <form action="{{ route('cart.applyCoupon') }}" class="py-4" method="POST">
        @csrf
        <label for="coupon" class="mr-2">{{ __('cart.have_coupon') }}</label>
        <input id="coupon" type="text" name="coupon_code" placeholder="{{ __('cart.coupon_placeholder') }}" class="p-2 border rounded">
        <button type="submit" class="px-4 py-2 text-white bg-green-600 rounded">{{ __('cart.apply') }}</button>
    </form>
