...
@section('content')
    <div class="bg-white dark:bg-gray-800 mx-3 rounded-md p-5  overflow-scroll scroll scroll-m-0 scrollbar-hide">
        ...
        <div class="grid grid-cols-8 text-left text-md font-bold text-black dark:text-white">
            @foreach (['#', 'Name', 'Arabic Name', 'Price', 'Delivery Time', 'Description', 'Country', 'Cities', 'Action'] as $label)
                <div class="px-4 py-3 border-b border-gray-200">{{ $label }}</div>
            @endforeach
        </div>

        {{-- Create Row --}}
        <form action="{{ route('admin.shippingOptions.store') }}" method="POST" class="grid grid-cols-8 border-b"
            x-data="shippingRow()">
            @csrf
            ...
            <div class="px-4 py-3"><input name="name_ar" class="form-input w-full" value="{{ old('name_ar') }}"></div>
            <div class="px-4 py-3"><input name="price" type="number" step="0.01" class="form-input w-full"
                    value="{{ old('price') }}"></div>
            <div class="px-4 py-3"><input name="delivery_time" class="form-input w-full" value="{{ old('delivery_time') }}">
            </div>
            <div class="px-4 py-3"><input name="description" class="form-input w-full" value="{{ old('description') }}">
            </div>
            <div class="px-4 py-3">
            ...
            <div class="px-4 py-3 flex items-center justify-center">
                <button type="submit" title="Add">
                    <span
                        class="material-icons p-2 rounded-full dark:hover:bg-gray-500 hover:bg-gray-300 dark:text-white">add</span>
                </button>
            </div>
        </form>
        ...
        @foreach ($shippingOptions as $option)
            <form action="{{ route('admin.shippingOptions.update', $option->id) }}" method="POST" class="grid grid-cols-8"
                x-data="shippingRow('{{ $option->country }}', {{ Js::from($option->cities ?? []) }})">
                @csrf
                @method('PUT')

                <div class="px-4 py-3 font-medium dark:text-white">{{ $loop->iteration }}</div>
                <div class="px-4 py-3"><input name="name" class="form-input w-full"
                        value="{{ old('name', $option->name) }}"></div>
                <div class="px-4 py-3"><input name="name_ar" class="form-input w-full"
                        value="{{ old('name_ar', $option->name_ar) }}"></div>
                <div class="px-4 py-3"><input name="price" type="number" step="0.01" class="form-input w-full"
                        value="{{ old('price', $option->price) }}"></div>
                <div class="px-4 py-3"><input name="delivery_time" class="form-input w-full"
                        value="{{ old('delivery_time', $option->delivery_time) }}"></div>
                <div class="px-4 py-3"><input name="description" class="form-input w-full"
                        value="{{ old('description', $option->description) }}"></div>
                <div class="px-4 py-3">
                ...
                <div class="px-4 py-3 flex items-center justify-center gap-2">
                    <button type="submit" title="Save">
                        <span
                            class="material-icons p-2 rounded-full dark:hover:bg-gray-500 hover:bg-gray-300 dark:text-white">edit</span>
                    </button>
            </form>
            <form action="{{ route('admin.shippingOptions.destroy', $option->id) }}" method="POST" class="contents"
